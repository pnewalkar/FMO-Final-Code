...
describe('Simulation: Controller', function () {
    ...
    beforeEach(function () {
        ...
        angular.mock.module(function ($provide) {
            $provide.factory('simulationAPIService', function ($q) {
                ...
                function getRoutes(operationStateID, deliveryScenarioID) {
                    var deferred = $q.defer();
                    if(loadDeliveryRouteMockData.length > 0)
                    {
        ...
        angular.mock.inject(function (_referenceDataConstants_,_referencedataApiService_,_$q_,_$controller_, _simulationService_, _$rootScope_,_$stateParams_) {
    ...
    it('should be set isDeliveryRouteDisabled is `true` ', function() {
        ...
        expect(vm.isDeliveryRouteDisabled).not.toBe(false);
    });
    ...
    it('should be cal loadScenario when selectedRouteStatus method initialize', function() {
        spyOn(vm,'selectedRouteStatus');
        spyOn(vm,'loadScenario');

        vm.selectedRouteStatus();
        vm.loadScenario('first','second');

        expect(vm.selectedRouteStatus).toHaveBeenCalled();
        expect(vm.isDeliveryRouteDisabled).toBe(true);
        expect(vm.isDeliveryRouteDisabled).toBeDefined();
        expect(vm.loadScenario).toHaveBeenCalledWith('first','second');
    });
    
    it('should be return promise response when loadRouteLogStatus have been called', function(done) {

        var RouteStatusObj = [{"id":"9c1e56d7-5397-4984-9cf0-cd9ee7093c88","name":null,"value":"Live","displayText":null,"description":"Live"},{"id":"bee6048d-79b3-49a4-ad26-e4f5b988b7ab","name":null,"value":"Not Live","displayText":null,"description":"Not Live"}];
        var selectedRouteStatusObj = "9c1e56d7-5397-4984-9cf0-cd9ee7093c88";

         
        //Cal to Promise and return response
        simulationService.loadRouteLogStatus(RouteStatusObj,selectedRouteStatusObj).then(function (response) {
            expect(vm.RouteStatusObj).toBe(response);
            expect(vm.selectedRouteStatusObj).toBe(vm.RouteStatusObj[0].id);
    ...
    it('should be isDeliveryRouteDisabled `false` when scenarioChanged', function() {
        
        spyOn(vm,'scenarioChange');

        vm.scenarioChange();
        vm.isDeliveryRouteDisabled = false;

        expect(vm.scenarioChange).toHaveBeenCalled();
        expect(vm.isDeliveryRouteDisabled).toBe(false);

    });

    it('should be return promise response when loadScenario called', function() {
        var selectedRouteStatusObj = "9c1e56d7-5397-4984-9cf0-cd9ee7093c88";
        ...
        simulationService.loadScenario(selectedRouteStatusObj,selectedDeliveryUnitID).then(function (response) {
            ...
            expect(vm.deliveryRoute).toBe(response);
            expect(vm.deliveryRoute).toBeDefined();
            //done(); //Asyn call check
        });
    ...
    it('should be return promise undefined when loadScenario called', function(done) {
        var selectedRouteStatusObj = "";
        var selectedDeliveryUnitID = "b51aa229-c984-4ca6-9c12-510187b81050";
    ...
    it('should be return promise response when cal loadDeliveryRoute', function(done) {
