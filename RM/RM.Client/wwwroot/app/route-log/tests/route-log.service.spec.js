...
*/
    beforeEach(function () {
        ...
        module(function ($provide) {
            ...
            $provide.factory('referencedataApiService', function ($q) {
              function getSimpleListsReferenceData(argument) {
                    deferred = $q.defer();
            ...
            $provide.value('referenceDataConstants',{
                ...
                SenarioOperationState: { AppCategoryName: "SenarioOperationState", DBCategoryName: "Senario Operation State", ReferenceDataNames: [] }
       		  ...
       		  $provide.constant('CommonConstants', {RouteLogSelectionType:{Multiple:true}});
    ...
    it('should promise to return a success response once loadSelectionType method is called', function () {
      var deffer = $q.defer();
      var response = [{"id":"ffeb0dbc-d12b-e711-8735-28d244aef9ed","name":null,"value":"Single","displayText":null,"description":"Single"},{"id":"01ae2cc6-d12b-e711-8735-28d244aef9ed","name":null,"value":"Multiple","displayText":null,"description":"Multiple"}];
      var selectionTypeResult = [];

      spyOn(referencedataApiService,'getSimpleListsReferenceData').and.returnValue(deffer.promise);

      routeLogService.loadSelectionType();
      deffer.resolve(response);
      scope.$digest();

      expect(referencedataApiService.getSimpleListsReferenceData).toHaveBeenCalled();      
    });
    ...
    it('should promise to return a success response once loadRouteLogStatus method is called', function () {
        ...
        routeLogService.loadRouteLogStatus();
        deffer.resolve(response);
        scope.$digest();
    ...
    it('should be return true object when scenarioChange method called ', function() {    	    	
    	expect(routeLogService.scenarioChange(true)).toEqual({isDeliveryRouteDisabled: true,isShowMultiSelectionRoute: true});      
    });

    it('should be return false object when scenarioChange method called ', function() {            
      expect(routeLogService.scenarioChange()).toEqual({isDeliveryRouteDisabled: false,isShowMultiSelectionRoute: false});      
    });
    
    it('should promise to return a success response once loadScenario method is called', function () {
      var deffer = $q.defer();
      spyOn(routeLogAPIService,'getScenario').and.returnValue(deffer.promise);

      routeLogService.loadScenario();
      deffer.resolve();
      scope.$digest();

      expect(routeLogAPIService.getScenario).toHaveBeenCalled();
    });       

    it('should promise to return a success response once loadDeliveryRoute method is called', function () {
      var deffer = $q.defer();
      var operationStateID = 'ffeb0dbc-d12b-e711-8735-28d244aef9ed';
      var deliveryScenarioID = 'ffeb0dbc-d12b-e711-8735-28d244aef9ed';
      var selectionType =  true;
      var response = { "deliveryRoute": true, "multiSelectiondeliveryRoute": true };
      spyOn(routeLogAPIService,'getRoutes').and.returnValue(deffer.promise);

      routeLogService.loadDeliveryRoute(operationStateID, deliveryScenarioID, selectionType);
      deffer.resolve(response);
      $rootScope.$apply();

      expect(routeLogAPIService.getRoutes).toHaveBeenCalled();
    });
    ...
    it('should promise to return a success response once deliveryRouteChange method is called', function () {
      ...
      var response = { "deliveryRoute": true, "multiSelectiondeliveryRoute": true };
      spyOn(routeLogAPIService,'getRouteDetailsByGUID').and.returnValue(deffer.promise);

      routeLogService.deliveryRouteChange(selectedRouteID);
      deffer.resolve(response);
      $rootScope.$apply();

      expect(routeLogAPIService.getRouteDetailsByGUID).toHaveBeenCalled();
      expect(routeLogAPIService.getRouteDetailsByGUID).toHaveBeenCalledWith('ffeb0dbc-d12b-e711-8735-28d244aef9ed');
    });

    it('should promise to return a success response once generateRouteLogSummary method is called', function () {
      var deffer = $q.defer();
      var deliveryRoute = true;
      var response = { "deliveryRoute": true, "multiSelectiondeliveryRoute": true };
      spyOn(routeLogAPIService,'generateRouteLogSummary').and.returnValue(deffer.promise);

      routeLogService.generateRouteLogSummary(deliveryRoute);
      deffer.resolve(response);
      $rootScope.$apply();

      expect(routeLogAPIService.generateRouteLogSummary).toHaveBeenCalled();
      expect(routeLogAPIService.generateRouteLogSummary).toHaveBeenCalledWith(true);
    });

    it('should promise to return a success response once generatePdf method is called', function () {
      var deffer = $q.defer();
      var pdfFilename = 'selectRoute.pdf';
      var response = { "deliveryRoute": true, "multiSelectiondeliveryRoute": true };
      spyOn(routeLogAPIService,'generatePdf').and.returnValue(deffer.promise);

      routeLogService.generatePdf(pdfFilename);
      deffer.resolve(response);
      $rootScope.$apply();

      expect(routeLogAPIService.generatePdf).toHaveBeenCalled();
      expect(routeLogAPIService.generatePdf).toHaveBeenCalledWith('selectRoute.pdf');
    });
