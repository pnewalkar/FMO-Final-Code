describe('Route-Log: Controller', function () {
    ...
    beforeEach(function () {
        ...
        angular.mock.module(function ($provide) {
            $provide.factory('routeLogService', function ($q) {
                ...
                function scenarioChange() {
                    deferred = $q.defer();
                     deferred.resolve(scenarioChangeMockData);
                    return deferred.promise;
                }
        ...
        angular.mock.inject(function (_$q_,$controller, routeLogService, _$rootScope_,items) {
    ...
    it('should be return object when loadScenario called and responce length > 0', function(done) {
        ...
        */
        ...
        mockService.loadScenario(arg1,arg2).then(function(res) {
        ...
        //$rootScope.$apply(); //scope is digest life cycle


    });

    it('should be return undefined when loadScenario called and responce blank', function(done) {

        mockService.loadScenario().then(function(res) {
    ...
    it('should be delivery route disabled when scenario changed', function(done) {

        mockService.scenarioChange("Single").then(function(result){
            controller.isDeliveryRouteDisabled = result.isDeliveryRouteDisabled;
            controller.isShowMultiSelectionRoute  = result.isShowMultiSelectionRoute;

            expect(controller.isDeliveryRouteDisabled).toBe(result.isDeliveryRouteDisabled);
            expect(controller.isShowMultiSelectionRoute).toBe(result.isShowMultiSelectionRoute);
            done(); //Check for Async call
        });
    ...
    it('should be return promise response when called loadDeliveryRoute method', function(done) {
        ...
        mockService.loadDeliveryRoute(operationStateID,deliveryScenarioID).then(function(response){

            controller.multiSelectiondeliveryRoute = response[0].multiSelectiondeliveryRoute;
            controller.deliveryRoute = response[0].deliveryRoute; 

            expect(controller.multiSelectiondeliveryRoute).toBe(response[0].multiSelectiondeliveryRoute);
            expect(controller.deliveryRoute).toBe(response[0].deliveryRoute); 
    ...
    it('Should be delivery route change method return promise result', function (done) {

        var selectedRouteValue = "d8053df2-1aad-40c8-bbf6-de0ae783e772";
        /*spyOn(mockService, 'deliveryRouteChange').and.callFake(function() {

            return deliveryRouteChangeMockData;
        });*/

        mockService.deliveryRouteChange(selectedRouteValue).then(function(response){
            controller.routeDetails = response; 
            expect(controller.routeDetails).toBe(response);
            done();
        });
